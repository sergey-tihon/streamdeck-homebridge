...
let getCloseReason(event: Browser.Types.CloseEvent) =
    ...
    | 1010 ->
        ...
        + event.reason
    | 1011 ->
        "Internal Server Error. Connection closed because it encountered an unexpected condition that prevented it from fulfilling the request."
    | 1015 ->
...
let createWebSocket inPort =
    ...
    websocket.onerror <- fun event -> console.warn($"WEBOCKET ERROR: {event}")
...
let addDynamicStyles(clrs: Dto.ColorInfo) =
    ...
    let mouseDownColor =
        if isNull clrs.mouseDownColor then
            fadeColor(clrs.highlightColor, -100)
        else
            clrs.mouseDownColor
